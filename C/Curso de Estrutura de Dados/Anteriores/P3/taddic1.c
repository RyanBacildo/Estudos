...
/* Função de hashing: transforma a chave em um índice do array de conteúdo. */
int fh(t_chave key) {
  int soma = 0;
...
Dic adDic(Dic d, t_chave k, t_dado dado) {
  ...
  // Verificando se a chave k já existe na lista (alteração)
  item = find_item(lst, k);

  if (item == NULL) { // INCLUI NOVO PAR K, DADO
    item = (t_item)malloc(sizeof(struct item_dic));
    item->key = k;
    item->valor = dado;
    appendLista(lst, item);
  } else { // SUBSTITUI O DADO DO K JÁ EXISTENTE PELO NOVO DADO.
    item->valor = dado;
  }
...
// QUANTIDADE DE CHAVES CADASTRADAS NO DICIONÁRIO
int lenDic(Dic d) {
  int total_ks = 0;
  Lista lst;
  for (int i = 0; i < MAX; i++)
...
// imprime o tamanho de todas as listas num vetor de listas
